...
export const deleteEvent = async (event_id: number) => {
	...
	try {
		...
		const { data: imageUrl, error: imageError } = await supabase
			...
			.single();
		if (imageError)
			throw new Error(`Failed to retrieve image URL: ${imageError.message}`);
		...
		if (youtubeLinksError)
			throw new Error(
				`Failed to retrieve YouTube links: ${youtubeLinksError.message}`,
			);
		...
		if (youtubeLinks && youtubeLinks.length > 0) {
			for (const link of youtubeLinks) {
				...
				if (deleteTagsError)
					throw new Error(
						`Failed to delete YouTube tags: ${deleteTagsError.message}`,
					);
			}
			...
			if (deleteLinksError)
				throw new Error(
					`Failed to delete event YouTube links: ${deleteLinksError.message}`,
				);
			...
			for (const link of youtubeLinks) {
				...
				if (deleteYTLinksError)
					throw new Error(
						`Failed to delete YouTube links: ${deleteYTLinksError.message}`,
					);
			}
		}
		...
		if (deleteTagsError)
			throw new Error(
				`Failed to delete event tags: ${deleteTagsError.message}`,
			);
		...
		const { error: deleteEventError } = await supabase
			...
			.eq("event_id", event_id);
		if (deleteEventError)
			throw new Error(`Failed to delete event: ${deleteEventError.message}`);
		console.log(
			`All related data for event ID ${event_id} have been deleted successfully.`,
		);
	} catch (error) {
