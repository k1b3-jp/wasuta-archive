...
export default async function handler(
	...
	if (
		...
		return res
			.status(400)
			.json({ error: "取り下げ理由を3〜500文字で入力してください" });
	if (isRateLimited(`milestones_withdraw:${user.id}`, 10))
		return res.status(429).json({ error: "rate limited" });
	try {
		const { error } = await supabase.rpc("withdraw_milestone", {
			milestone_to_withdraw: milestoneId,
