...
- モデル: 認証ユーザーに対してイベント・タグ・動画・画像の登録/更新を許可
- 価値: コミュニティ協働によりデータを継続的に拡充・修正できることが本サービスの中核価値
- 前提: 未認証ユーザーは閲覧のみ。認証済みユーザーに広い編集権限を付与する設計
- 補助的な安全策
  - 監査と責任追跡: 変更は API 経由で実施し、監査ログで追跡可能。`created_by` / `updated_by` をテーブルに追加して可視化
  - レート制限と健全性担保: API でユーザー/IP ごとの投稿頻度制限を導入
  - バリデーションとモデレーション: 型・値検証、URL の正当性チェックを強化。不適切投稿は管理者が削除可能（削除権限は原則管理者に限定）
  - ストレージ制約: `event_pics` は画像 MIME タイプのみに制限、サイズ上限を設定
- 注意点
  - 認証ユーザーは更新可能。削除は原則管理者のみ（例外: `storage.objects` は削除も認証ユーザー可）
