...
grant select on public.events, public.event_tags, public.event_tag_names to anon, authenticated;
grant insert, update, delete on public.events, public.event_tags, public.event_tag_names to authenticated;
...
create policy "event_tag_names_select_public" on public.event_tag_names for select to public using (true);
create policy "event_tag_names_write_admin_only" on public.event_tag_names for insert to authenticated with check (public.is_admin(auth.uid()));
create policy "event_tag_names_update_admin_only" on public.event_tag_names for update to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));
create policy "event_tag_names_delete_admin_only" on public.event_tag_names for delete to authenticated using (public.is_admin(auth.uid()));
...
create policy "storage_objects_admin_write_only" on storage.objects for insert to authenticated
  with check (bucket_id = 'event_pics' and public.is_admin(auth.uid()));
create policy "storage_objects_admin_update_only" on storage.objects for update to authenticated
  using (bucket_id = 'event_pics' and public.is_admin(auth.uid()))
  with check (bucket_id = 'event_pics' and public.is_admin(auth.uid()));
create policy "storage_objects_admin_delete_only" on storage.objects for delete to authenticated
  using (bucket_id = 'event_pics' and public.is_admin(auth.uid()));
